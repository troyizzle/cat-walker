...
class ApplicationFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def labelled_check_box(method, label_options: {}, input_options: {})
    ...
    tag.div(class: "flex items-center") do
      label(method, merge_options(label_options, class: "mr-1")) +
        check_box(method, merge_options(input_options, class: "form-checkbox"))
    end
  end
  ...
  def rich_submit(value = nil, options = {})
    options[:class] =
      "w-full text-center py-3 rounded hover:bg-indigo-900 bg-indigo-600 text-white focus:outline-none my-1"
